...
var (
	...
	ErrWriteAfterFlush = errors.New("unused") // 这个不理解是什么含义
)
...
type ResponseWriter interface {
	...
	// Content-Length header is added automatically. // 不需要flush操作就可以
	//
	// Depending on the HTTP protocol version and the client, calling
	// Write or WriteHeader may prevent future reads on the // 可能阻止未来读body？？？？
	// Request.Body. For HTTP/1.x requests, handlers should read any
	// needed request body data before writing the response. Once the // 一旦header被写进，re body不能读了
	// headers have been flushed (due to either an explicit Flusher.Flush // 这个可以后来看下
	// call or writing enough data to trigger a flush), the request body
	// may be unavailable. For HTTP/2 requests, the Go HTTP server permits
	// handlers to continue to read the request body while concurrently
	// writing the response. However, such behavior may not be supported
	// by all HTTP/2 clients. Handlers should read before writing if // server 不可以同时读写
	// possible to maximize compatibility. // 最大化兼容性
	...
	// send error codes. // 主要是为了报错误，正常的都是ok
	//
	// The provided code must be a valid HTTP 1xx-5xx status code.
	// Only one header may be written. Go does not currently
	// support sending user-defined 1xx informational headers,
	// with the exception of 100-continue response header that the
	// Server sends automatically when the Request.Body is read. // server自动送过去
...
// A conn represents the server side of an HTTP connection.  // 代表http连接的服务器端
type conn struct {
	// server is the server on which the connection arrived.
	// Immutable; never nil. // 从不为nil
	server *Server

	// cancelCtx cancels the connection-level context. // 取消用的，echo的context属于自己定义的，而不是这个
	cancelCtx context.CancelFunc

	// rwc is the underlying network connection. // 网络连接指的是tcp层面的东西了   要和tcp区别开
	// This is never wrapped by other types and is the value given out
	// to CloseNotifier callers. It is usually of type *net.TCPConn or // 这里很重要
	// *tls.Conn.
	rwc net.Conn // 这是一个接口

	// remoteAddr is rwc.RemoteAddr().String(). It is not populated synchronously // 不是同步填充的
...
// A response represents the server side of an HTTP response. // 代表了 http的response
