...
type Client struct {
	...
	Transport RoundTripper // 这里是面向接口编程了，transport 实现了这个接口，所以面向接口编程，而不是面向接口实现编程
...
func (c *Client) deadline() time.Time {
	if c.Timeout > 0 {
		return time.Now().Add(c.Timeout) // 返回一个加上timeout的值
	}
	return time.Time{}
}
...
// (typically Transport) may not be able to re-use a persistent TCP //tk 这里有潜在的http连接,能够重新利用，在开启长连接或者默认开启的时候
// connection to the server for a subsequent "keep-alive" request.
//
// The request Body, if non-nil, will be closed by the underlying
// Transport, even on errors. // 这里会自己关闭,这里就厉害了,所以request的body不用管
//
// On error, any Response can be ignored. A non-nil Response with a
// non-nil error only occurs when CheckRedirect fails, and even then
// the returned Response.Body is already closed.
//
// Generally Get, Post, or PostForm will be used instead of Do. //这里要注意
//
...
//
// Any returned error will be of type *url.Error. The url.Error //TK 我可以用这个方法检测
// value's Timeout method will report true if request timed out or was
// canceled.
...
// tk 有个问题，这里如何区分get  post的呢，好像是不区分的，只是post带数据，而且只有两个，太简单了
...
// so that it can copy headers into the upcoming Request. tk 这里应该是为了处理重定向
	...
	return func(req *Request) {
		...
		if c.Jar != nil && icookies != nil {
			...
			if changed {
				...
				ireqhdr.Set("Cookie", strings.Join(ss, "; ")) //tk 把cookie放进header
			}
		}

		// Copy the initial request's Header values
		// (at least the safe ones).tk 这里是为了保证安全吗
		...
		preq = req // Update previous Request with the current request // 每次都是更新成最近的
	}
}
